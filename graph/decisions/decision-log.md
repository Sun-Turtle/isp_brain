...
## 2026-05-24: Bank account — Tide (CLG-compatible)

**Context**: Sunturtle needed a business bank account. Starling and Monzo Business both rejected the CLG structure — their onboarding flows expect share capital fields. Confirmed on Reddit that Tide accepts CLGs. **Decision**: Open a business account with Tide, a UK fintech with CLG-compatible onboarding. **Rationale**: Only option that worked for the CLG structure, free tier, FSCS-protected via ClearBank, Open Banking feeds integrate with QuickFile, fast online setup. **Alternatives**: Starling (rejected — no CLG support), Monzo Business (rejected — same issue), high-street banks (accept CLGs but require in-person visits and weeks of processing). **Details**: See [Bank Account: Tide](company-bank-tide.md).

## 2026-05-28: Director's Loan Account

**Context**: Startup costs (incorporation, domain, subscriptions) are paid by the director personally. As a CLG with no share capital, these must enter the books as a director's loan, not shares. The QuickFile account already had a Director's Loan Account (BankId 53414, NominalCode 1201, type LOAN) sitting at £0.00 balance. **Decision**: Use that existing account to track all director-funded expenses, with reimbursement from Tide current account when the company has revenue. **Rationale**: Clean personal/company separation from day one, proper audit trail for Companies House, matches CLG structure requirement (no share capital = loan, not equity). **Alternatives**: Creating a new nominal ledger (unnecessary — bank-type LOAN gives dashboard visibility and transaction tagging), logging as personal expenses (no separation, tax risk). **Details**: See [Director's Loan Account](company-directors-loan-account.md).
